...
date: today
date-format: long
abstract: "This paper analyzes Toronto's Bike Share ridership and cycling infrastructure development from 2017 to 2023 using open data. Findings reveal a nearly fivefold increase in annual Bike Share rides, from 1.2 million to 5.7 million, outpacing the growth of cycling infrastructure. While bikeways increased from 539 in 2001 to over 1,400 by 2023, the expansion rate lags behind ridership growth. The analysis highlights a shift towards safer infrastructure, with increased focus on protected lanes. These trends underscore the need for accelerated and strategic expansion of Toronto's cycling network to meet growing demand and ensure safe, accessible cycling options for residents."
format: pdf
...
# Introduction
As cities worldwide grapple with issues of congestion, pollution, and the need 
for sustainable transportation, many are turning to cycling as a potential 
solution. Toronto, Canada's largest city, has been at the forefront of this 
trend, implementing a bike-sharing program and expanding its cycling 
infrastructure to encourage more residents to adopt this eco-friendly mode of 
transport.
...
bikeShare_data <- bikeShare_data %>% filter(year(start_date) <= 2023)

bikeWay_data <- read_csv(here::here("data/analysis_data/bikeway_data.csv"))
bikeWay_data <- bikeWay_data %>% filter(INSTALLED <= 2023)
...
These two datasets were chosen as they represent the most up-to-date and accurate information available on Toronto's cycling infrastructure and Bike Share usage. Other potentially relevant datasets, such as bike lanes measured in kilometers or total bike counts, were considered but ultimately not used due to being outdated or no longer maintained.

When analyzed in tandem, these datasets provide a comprehensive picture of how Toronto's cycling infrastructure has developed in relation to the growing usage of the Bike Share program. This analysis aims to shed light on whether the city's bikeway infrastructure is keeping pace with the increasing demand for cycling as a mode of transportation in Toronto.
...
ggplot(combined_data, aes(x = year)) +
  ...
  scale_y_continuous(
    name = "Total Annual Bike Share Rides", 
  ...
  scale_fill_manual(values = c("Bikeways" = "#2ca02c")) +
  labs(
    x = "Year",
    color = NULL,
    fill = NULL
...
The analysis of Toronto's Bike Share ridership and cycling infrastructure development from 2017 to 2023 reveals a complex interplay between infrastructure investment and cycling adoption in the city. This discussion will explore the implications of our findings, addressing the central question of whether Toronto's bikeway infrastructure is keeping pace with the growth in Bike Share ridership.
...
3. Data-Driven Planning: Continued monitoring and analysis of ridership patterns and infrastructure usage can inform more targeted and effective cycling network expansion.
...
1. Geographic Distribution: This study did not examine the spatial distribution of Bike Share usage and infrastructure. Future research could explore how infrastructure development in specific neighborhoods correlates with local Bike Share usage.
2. Quality of Infrastructure: Our analysis focused on the quantity of infrastructure rather than its quality or connectivity. Future studies could incorporate measures of infrastructure quality and network connectivity to provide a more comprehensive understanding of the cycling landscape in Toronto.
3. User Demographics: Incorporating data on user demographics could provide insights into who is benefiting from the Bike Share program and where there might be opportunities for greater inclusivity.
