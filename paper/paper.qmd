...
This study utilizes two primary datasets: Toronto's Bike Share ridership data [@bikeShareData] and the city's Cycling Network (Bikeways) data [@bikeWayData]. Both datasets were obtained from the City of Toronto's Open Data Portal [@torontoOpenData], providing a comprehensive view of cycling infrastructure from 2001 to 2024, and Bike Share ridership in the city from 2017 to 2023. These datasets were selected for their up-to-date and comprehensive nature, offering the most accurate available information on Toronto's cycling infrastructure and Bike Share usage.
...
We cleaned the data by downloading all CSV files, extracting required columns (trip ID and start date), and performing necessary cleaning operations. This process involved standardizing column names across files, standardizing data types, and aggregating the data into monthly total ridership figures. Additionally, any incomplete or invalid entries were removed ruing the cleaning process to ensure data integrity. This process resulted in a cleaned dataset providing a comprehensive view of Bike Share usage from January 2017 to December 2023.

@tbl-sample-bikeshare presents a sample of the processed dataset, showcasing the year and month of observation along with the total number of rides for that month. This granular monthly data allows for analysis of seasonal trends and year-over-year growth in ridership.
...
This summary reveals a consistent increase in total annual rides from 1,190,320 in 2017 to 5,713,141 in 2023, representing nearly a fivefold increase over seven years. The average monthly rides have grown correspondingly, indicating a substantial expansion of the bike shares program's usage.

## Cycling Network (Bikeways) Data
The Cycling Network data, sourced from the City of Toronto's Transportation Services division, provides detailed information about the city's bikeways. This includes their construction year, any upgrades, and the type of bikeway. @tbl-sample_bikeway offers a sample of this dataset.
...
1. Filtering out Constructed year and Upgraded year entries with invalid years (e.g., year labeled as "1") or missing years.
2. Standardizing bikeway type classifications into the three main categories (Protected Lanes, On-Road Lanes, Shared Roadways).
